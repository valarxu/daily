...
export const Card: React.FC<CardProps> = ({ data, onUpdate, onDelete }) => {
  const { tasks: availableTasks, addTask } = useStore();
        ...
        <div className="flex flex-wrap gap-2">
            {cardTasks.map(task => (
                <button
                    ...
                    className={cn(
                        "px-4 py-1.5 rounded-full text-sm font-medium text-white shadow-sm transition-all transform hover:scale-105 active:scale-95",
                        selectedTaskId === task._id ? "ring-2 ring-offset-2 ring-gray-400 scale-105" : "opacity-90 hover:opacity-100"
                    )}
            ...
            {!isAddingTask ? (
                ...
                <div className="flex items-center gap-2 w-full mt-2 p-3 bg-gray-50 rounded-xl border border-gray-200 animate-in fade-in slide-in-from-top-2">
                    <input 
                        type="text" 
                        value={newTaskName}
                        onChange={(e) => setNewTaskName(e.target.value)}
                        placeholder="Task name"
                        className="flex-1 text-sm bg-white px-3 py-1.5 rounded border border-gray-200 focus:border-blue-500 outline-none"
                        autoFocus
